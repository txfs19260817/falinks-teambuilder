    ...
    <>
      ...
      <label htmlFor={AppConfig.toolboxIDs.historyModal} className="modal-right modal" ref={modalRef}>
        <label className="modal-box" dir="rtl" htmlFor="">
          <label htmlFor={AppConfig.toolboxIDs.historyModal} className="btn btn-circle btn-sm absolute left-1.5 top-1.5">
            ✕
          </label>
          <div className="ml-2 mt-2" dir="ltr">
            ...
            <ul className="steps steps-vertical">
              {teamState.history.map((change, idx) => (
                <li key={idx} className="step step-primary text-xs">
                  {change}
                </li>
              ))}
            </ul>
          ...
          <div className="modal-action">
            <button className="btn btn-primary btn-sm" onClick={() => teamState.clearHistory()}>
